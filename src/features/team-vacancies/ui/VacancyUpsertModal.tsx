...
export function VacancyUpsertModal({
  ...
  const rolesById = useMemo(
    () => new Map(catalogRoles.map(r => [r.id, r])),
    [catalogRoles]
  )

  const catalogSkills = useMemo(
    () =>
      catalogData?.skills?.filter(
        (s): s is { id: string; name: string } => Boolean(s?.id && s?.name)
      ) ?? [],
    [catalogData]
  )

  const catalogById = useMemo(
    () => new Map(catalogSkills.map(s => [s.id, s])),
    [catalogSkills]
  )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await upsertMutation.mutateAsync({
        ...
        desiredRoleIds:
          selectedRoleIds.length > 0 ? selectedRoleIds : undefined,
        desiredSkillIds:
        ...
        <div className="flex flex-col gap-m8">
          <div className="flex flex-col gap-m6">
            ...
            <ChipList>
              ...
              <Chip
                variant="secondary"
                icon={<Icon src="/icons/icon-plus/icon-plus-sm.svg" size="sm" color="secondary" />}
